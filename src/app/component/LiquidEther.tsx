...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      mouseMoved = false;
      coords = new THREE.Vector2();       // current position (normalized -1..1)
      coords_old = new THREE.Vector2();   // last position
      diff = new THREE.Vector2();         // movement delta
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      /**
       * Apply an external force (from mouse/touch)
       */
      addForce(position: THREE.Vector2, movement: THREE.Vector2, strength: number, radius: number) {
        const uniforms = (this.mouse.material as THREE.RawShaderMaterial).uniforms;

        uniforms.force.value.copy(movement).multiplyScalar(strength);
      ...
      /**
       * Old auto-update mode (keeps compatibility)
       */
      update(props: any = {}) {
        const forceX = (Mouse.movement.x / 2) * (props.mouse_force || 0);
        const forceY = (Mouse.movement.y / 2) * (props.mouse_force || 0);
        const cursorSize = props.cursor_size || 0;

        const uniforms = (this.mouse.material as THREE.RawShaderMaterial).uniforms;
        uniforms.force.value.set(forceX, forceY);
        uniforms.center.value.copy(Mouse.position);
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      render() {
        ...
        // 🔹 Update mouse (updates coords + diff)
        Mouse.update();

        // 🔹 Apply mouse as external force
        if (this.output?.simulation?.externalForce) {
          this.output.simulation.externalForce.update({
